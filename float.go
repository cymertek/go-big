...
func (z *Float) Prune(x *Float) *Float {
	...
	case finite:
		...
		// 1 <= |x| < +Inf
		// remove integer portion of the mantisa
		if exp > _W {
			n := int(exp / _W)
			m = m[:len(m)-n]
			exp -= uint(n * _W)
		...
		// shift may have canceled out the mantisa
		if len(m) == 0 {
			x.acc = Exact
			x.form = zero
			x.neg = false
		...
		// normalize the mantisa and exponent
		fnorm(m)
		x.exp = -int32(s)
		x.mant = m
